...
export default function AdminHeroImages() {
  ...
  const handleUpload = async () => {
    if (selectedFiles.length === 0) { toast.error('Select at least one image'); return; }
    setUploading(true);
    let count = 0;
    for (let i = 0; i < selectedFiles.length; i++) {
      try {
        const res = await uploadAPI.upload(selectedFiles[i]);
        await heroImageAPI.create({ imageUrl: res.data.url, displayOrder: images.length + i, active: true });
        count++;
      } catch {
    ...
    setUploading(false);
    if (count > 0) toast.success(`${count} image${count > 1 ? 's' : ''} added!`);
    setShowModal(false);
    load();
  };
  ...
  const handleDelete = async (id) => {
    ...
    try { await heroImageAPI.delete(id); toast.success('Removed'); load(); }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-white font-display text-3xl font-bold">Hero Images</h1>
          <p className="text-white/40 text-sm mt-1">
            {images.length} image{images.length !== 1 ? 's' : ''} — slides every 10 seconds on the home page
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center text-white/30 py-20">
          ...
          <p className="text-sm mt-1">Add images to replace the default hero background</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {images.map((img, i) => (
            <div key={img.id} className="group relative rounded-xl overflow-hidden bg-gray-900 aspect-video">
              <img
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-lg">
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-display text-white text-xl font-bold">Add Hero Images</h2>
