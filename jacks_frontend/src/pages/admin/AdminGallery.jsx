...
export default function AdminGallery() {
  ...
  const existingCategories = [
    ...new Set([...DEFAULT_CATEGORIES, ...images.map(i => i.category).filter(Boolean)])
  ];
  ...
  const handleUpload = async () => {
    ...
    const category = catMode === 'new'
      ? newCategory.trim().toLowerCase().replace(/\s+/g, '-')
      : selectedCategory;
    if (!category) { toast.error('Please enter a category name'); return; }
    ...
    setUploading(false);
    if (successCount > 0) toast.success(`${successCount} image${successCount > 1 ? 's' : ''} added!`);
    setShowModal(false);
    load();
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={openModal} className="btn-primary flex items-center gap-2"><HiPlus size={18} /> Add Images</button>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-display text-white text-xl font-bold">Add Images</h2>
            ...
            <div className="space-y-4">
              ...
              {/* Caption */}
              <div>
                <label className="text-white/50 text-xs uppercase tracking-wider mb-1 block">Caption (applied to all)</label>
                <input value={caption} onChange={e => setCaption(e.target.value)} placeholder="Optional caption" className={inputCls} />
              </div>

              {/* File picker */}
              <div>
                <label className="text-white/50 text-xs uppercase tracking-wider mb-2 block">Images *</label>
                <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-white/20 hover:border-pub-gold rounded-xl py-6 cursor-pointer transition-colors">
                  ...
                  <span className="text-white/50 text-sm">Click to select images (multiple allowed)</span>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  {uploading ? `Uploading... (${selectedFiles.length} files)` : `Upload ${selectedFiles.length || ''} Image${selectedFiles.length !== 1 ? 's' : ''}`}
                </button>
              </div>
            </div>
          </div>
