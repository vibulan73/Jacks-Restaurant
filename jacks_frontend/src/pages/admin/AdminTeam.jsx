...
export default function AdminTeam() {
  ...
  const openEdit = (m) => {
    setEditing(m);
    setForm({ name: m.name, position: m.position, imageUrl: m.imageUrl || '', displayOrder: m.displayOrder ?? '' });
    setImageFile(null);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-white font-display text-3xl font-bold">Team Members</h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {members.map(m => (
            <div key={m.id} className="bg-gray-900 border border-white/10 rounded-2xl overflow-hidden group">
              ...
              <div className="p-4">
                <h3 className="text-white font-display font-bold text-lg leading-tight">{m.name}</h3>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {/* Image upload */}
              <div>
                <label className="text-white/50 text-xs uppercase tracking-wider mb-2 block">Photo</label>
              ...
              {/* Position */}
              <div>
                <label className="text-white/50 text-xs uppercase tracking-wider mb-1 block">Position *</label>
                <input
                  value={form.position}
              ...
              {/* Display order */}
              <div>
                <label className="text-white/50 text-xs uppercase tracking-wider mb-1 block">Display Order</label>
                <input
                  type="number"
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {uploading ? 'Uploading...' : saving ? 'Saving...' : editing ? 'Save Changes' : 'Add Member'}
                </button>
              </div>
            </form>
          </div>
