...
export default function AdminPromotions() {
  ...
  const load = () =>
    promotionAPI.getAll().then(r => setPromotions(r.data)).catch(console.error).finally(() => setLoading(false));
  ...
  const onSubmit = async (data) => {
    try {
      const payload = {
        ...
        startDateTime: promoType === 'SPECIAL' && data.startDateTime ? data.startDateTime : null,
        endDateTime: promoType === 'SPECIAL' && data.endDateTime ? data.endDateTime : null,
      };
      if (editing) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-white font-display text-3xl font-bold">Promotions & Specials</h1>
          <p className="text-white/40 text-sm mt-1">{promotions.length} total promotions</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => openAdd('DAILY')} className="btn-primary flex items-center gap-2 text-sm">
            <FaSun size={14} /> Add Daily Special
          </button>
          <button onClick={() => openAdd('SPECIAL')} className="flex items-center gap-2 px-4 py-2 border border-pub-gold/50 text-pub-gold rounded-sm hover:bg-pub-gold/10 text-sm font-semibold uppercase tracking-wider transition-all">
        ...
        <>
          ...
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-white font-display text-xl font-semibold">Daily Specials</h2>
              <span className="text-white/30 text-sm">({daily.length})</span>
            </div>
            {daily.length === 0 ? (
              <div className="text-white/30 text-sm py-4">No daily specials. Add one above.</div>
            ) : (
              <div className="space-y-6">
                {DAYS.map(day => {
                  const dayItems = sortedDaily.filter(p => p.dayOfWeek === day);
                  if (dayItems.length === 0) return null;
                  return (
                    <div key={day}>
                      <p className="text-white/40 text-xs uppercase tracking-widest mb-3 font-semibold">{day}</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {dayItems.map(p => <PromoCard key={p.id} p={p} onEdit={openEdit} onDelete={handleDelete} />)}
                      </div>
                    </div>
                  );
                })}
                ...
                {(() => {
                  ...
                  return (
                    <div>
                      <p className="text-white/40 text-xs uppercase tracking-widest mb-3 font-semibold">Unassigned</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {unassigned.map(p => <PromoCard key={p.id} p={p} onEdit={openEdit} onDelete={handleDelete} />)}
                      </div>
                    </div>
                  );
                })()}
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-white font-display text-xl font-semibold">Featured Specials</h2>
              <span className="text-white/30 text-sm">({special.length})</span>
            </div>
            {special.length === 0 ? (
              <div className="text-white/30 text-sm py-4">No featured specials. Add one above.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {special.map(p => <PromoCard key={p.id} p={p} onEdit={openEdit} onDelete={handleDelete} />)}
              </div>
            )}
          </div>
        </>
          ...
          <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-2">
                {promoType === 'DAILY' ? <FaSun className="text-blue-400" /> : <FaStar className="text-purple-400" />}
                <h2 className="font-display text-white text-xl font-bold">
                  {editing ? 'Edit' : 'New'} {promoType === 'DAILY' ? 'Daily Special' : 'Featured Special'}
                </h2>
              </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {promoType === 'DAILY' && (
                <div>
                  <label className="text-white/50 text-xs uppercase tracking-wider mb-2 block">Day of Week *</label>
                  ...
                  {dayOfWeek && (
                    <p className="text-pub-gold text-xs mt-2">Selected: {dayOfWeek}</p>
                  )}
                </div>
              )}
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="datetime-local" {...register('startDateTime')} className={inputCls} />
                    </div>
                    <div>
                      <label className="text-white/50 text-xs uppercase tracking-wider mb-1 block">End Date & Time</label>
                      <input type="datetime-local" {...register('endDateTime')} className={inputCls} />
                    </div>
                  </div>
                </>
              )}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={isSubmitting}
                  className="flex-1 btn-primary disabled:opacity-50 text-sm">{isSubmitting ? 'Saving...' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
...
function PromoCard({ p, onEdit, onDelete }) {
  const FALLBACK_IMG = 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=200&h=120&fit=crop';
  ...
  return (
    <div className="bg-gray-900 border border-white/10 rounded-xl overflow-hidden">
      ...
      <div className="p-4">
        ...
        {p.description && <p className="text-white/50 text-xs line-clamp-2 mb-2">{p.description}</p>}
        {!isDaily && p.endDateTime && (
          <p className="text-white/30 text-xs mb-2">Ends: {new Date(p.endDateTime).toLocaleString('en-AU', { dateStyle: 'short', timeStyle: 'short' })}</p>
        )}
        <div className="flex gap-2 mt-3">
          ...
          <button onClick={() => onDelete(p.id, p.title)}
